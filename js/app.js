...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 7. Scroll Blur-In Reveal Observer
    ...
    // 8.9 Royal Interactive Practice Deck Showcase Engine (With Lively 3D Animation & Progress Bar)
    const initServicesDeck = () => {
        const tabBtns = document.querySelectorAll('#service-tabs .tab-btn');
        const deckCards = document.querySelectorAll('#services-deck .deck-card');
        const counterNum = document.getElementById('deck-current-num');
        const prevBtn = document.getElementById('deck-prev-btn');
        const nextBtn = document.getElementById('deck-next-btn');
        const canvas = document.querySelector('.practice-studio-canvas');
        ...
        const showDeckCard = (index) => {
            if (index < 0) index = deckCards.length - 1;
            if (index >= deckCards.length) index = 0;
            ...
            // Update Right Deck Cards with smooth animation restart
            deckCards.forEach((card, idx) => {
                if (idx === currentIndex) {
                    card.classList.remove('active');
                    void card.offsetWidth; // Trigger DOM reflow to re-play 3D keyframe
        ...
        const startAutoplay = () => {
            ...
            autoPlayTimer = setInterval(() => {
                showDeckCard(currentIndex + 1);
            }, 8000); // Relaxed 8-second reading pace

            progressInterval = setInterval(() => {
                progressPercent += 1.25; // 100% / 80 steps = 1.25% per 100ms
        ...
        if (canvas) {
            canvas.addEventListener('mouseenter', stopAutoplay);
            canvas.addEventListener('mouseleave', startAutoplay);
        }

        // Start initial lively autoplay cycle
        startAutoplay();
    };
