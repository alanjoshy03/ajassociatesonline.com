...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCalculatorResults() {
        ...
        if (selectedEntity === 'individual') {
            ...
            features = [
                'Income Tax Return (ITR-1 / ITR-2 / ITR-3) E-Filing',
                'Form 26AS & AIS Tax Data Reconciliation',
                'Advance Tax Computation & Quarterly Estimation',
                'Capital Gains & Investment Tax Exemption Guidance',
                'Foreign Income & NRI Remittance Advisory',
                'Bank Interest & Dividend Income Tax Audit'
            ];
            ...
            if (selectedTurnover === 'under_20l') {
                ...
                tier = 'Starter Scope';
                features = [
                    'Annual Income Tax Return (ITR-3/4 Presumptive) Filing',
                    'Financial Statement Preparation (P&L & Balance Sheet)',
                    'Form 26AS / AIS Tax Reconciliation & TDS Claiming',
                    'Advance Tax Computation & Payment Schedule Advice',
                    'MSME / Udyam Registration & Compliance Support',
                    'Bank Statement Reconciliation & Ledger Review'
                ];
            } else {
                packageName = 'Complete Proprietorship & GST Compliance';
                tier = 'Growth Scope';
                features = [
                    'Monthly GST Return Filings (GSTR-1, GSTR-3B & GSTR-2B)',
                    'Income Tax & Statutory Tax Audit Compliance (Sec 44AB)',
                    'Form 26AS / AIS Reconciliation & Input Tax Credit (ITC) Verification',
                    'Quarterly TDS Computation, Payment & E-Filing (26Q)',
                    'Advance Tax Computation & Quarterly Tax Planning',
            ...
            features = [
                'LLP Form 11 & Form 8 Annual MCA Compliance Filings',
                'Partner Capital Account Accounting & Profit Distribution',
                'Monthly GST Filings & GSTR-2B Input Credit Matching',
                'Quarterly TDS Computation, Payment & E-Filing',
                'Form 26AS / AIS Reconciliation & Advance Tax Advice',
                'Statutory Tax Audit & Regulatory Representation'
            ];
        } else if (selectedEntity === 'pvt_ltd') {
            packageName = 'Pvt Ltd Corporate Compliance Retainer';
            tier = 'Executive Scope';
            features = [
                'MCA Annual Financial & Governance Filings (AOC-4 & MGT-7)',
                'Statutory Financial Audit Supervision & Board Resolutions',
                'Form 26AS / AIS & Input Tax Credit (ITC) Reconciliation',
                'Monthly GST Filings (GSTR-1, GSTR-3B) & Quarterly TDS (26Q)',
                'Advance Tax Computation & Corporate Tax Planning',
                'Director KYC (DIR-3 KYC) & Corporate Secretarial Maintenance'
            ];
        }

        if (selectedTurnover === '1cr_5cr' || selectedTurnover === 'above_5cr') {
            tier = 'Enterprise Scope';
            features.push('Bank CMA Data & Loan Syndication Proposal Support');
        }
        ...
        if (featuresListEl) {
            featuresListEl.innerHTML = features.map(f => `<li><i class="fas fa-check-circle"></i> <span>${f}</span></li>`).join('');
        }
...
// Universal Mailto & Clipboard Toast Handler (Fixes mailto silently failing on desktop browsers without configured mail apps)
window.handleMailtoClick = function (event, email, subject) {
    if (event) event.preventDefault();

    const cleanSubject = subject || 'Consultation Inquiry - AJ Associates';
    const mailtoUrl = `mailto:${email}?subject=${encodeURIComponent(cleanSubject)}`;
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${email}&su=${encodeURIComponent(cleanSubject)}`;

    // 1. Copy email to clipboard silently
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(email).catch(() => { });
    }

    // 2. Trigger mailto protocol
    window.location.href = mailtoUrl;

    // 3. Display interactive toast notification with webmail fallback
    showEmailToast(email, gmailUrl);
};

function showEmailToast(email, gmailUrl) {
    let toast = document.getElementById('email-toast-notification');
    if (!toast) {
        toast = document.createElement('div');
        toast.id = 'email-toast-notification';
    ...
    toast.innerHTML = `
        <div class="toast-content">
            ...
            <div class="toast-text">
                <strong>Opening Mail Client...</strong>
                <span>Copied <code>${email}</code> to clipboard.</span>
            </div>
            <a href="${gmailUrl}" target="_blank" rel="noopener noreferrer" class="toast-gmail-btn">
                <i class="fab fa-google"></i> Open in Gmail
            </a>
            <button type="button" class="toast-close-btn" onclick="this.parentElement.parentElement.classList.remove('active')">&times;</button>
        </div>
    `;
